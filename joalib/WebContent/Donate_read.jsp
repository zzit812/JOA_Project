...
.donateComments>a:nth-child(4), .donateComments>a:nth-child(5),
	.donateComments>a:nth-child(6), .donate_small_comment_list a:nth-child(2),
	.donate_small_comment_list a:nth-child(3) {
	float: right;
	opacity: 70%;
	font-size: 13px;
	margin-right: 7px;
		...
		<div id="cont_size">
			...
			<div id="cont_1_size">
				...
				<div id="write_box">
					...
					<div id="donate_text">
						<img
							src="<%= request.getContextPath() +"/donateImage/"+dto.getDonate_attach() %>" />
				...
				<form name="btns" method="post">
					...
					<div id="donateApplicationBtn">
						<input type="hidden" name="donateApplicationMember"
							value="<%= member_id %>" /> <input type="hidden"
							name="donateWriter" value="<%= dto.getMember_id() %>" /> <input
							type="hidden" name="donate_no" value="<%= dto.getDonate_no() %>" />
						<%if(member_id != null && !member_id.equals(dto.getMember_id())){%>
						<input type="button" name="donateApplicationBtn"
							value="<%if( dao.DonateApllcationSelect(member_id, donate_no) > 0){	out.print("취소");	}else{	out.print("신청");	} %>" />
						<%} %>
					</div>
					<div id="donateApplicationCount">
						<!-- 도서나눔 신청한 사람 수 -->
						<% int count;
					...
					<div class="button">
						...
						<input type="button" value="목록"
							onClick="location.href='Donate_list.jsp?page_num=<%= page_num%>'" />
						<%	
							if ( member_id != null && member_id.equals(dto.getMember_id())) { 
						...
						<input type='button' value='수정'
							onClick="location.href='Donate_update.jsp?donate_no=<%= donate_no%>'" />
						<input type='button' value='삭제' onClick="removeCheck()" />

						<% }%>
					</div>

					<div id="donePopMask"></div>
					<div id="donePopBtn">
						<h1>
							회원 선택<a href="javascript:void(0);" onClick="">X</a>
						</h1>
						...
						<div>
							...
							</select> <input type="button" id="popMemSelectBtn" value="OKOK">
						</div>
					</div>
