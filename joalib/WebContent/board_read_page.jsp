		...
		<div id="cont_size">
			...
			<div id="cont_1_size">
				...
				<div id="write_box">
					<!-- 게시글 내용 -->
					<h2>
						<%out.print(article.getBoard_title());%>
					</h2>
					<div id="memberinfo">
						<div class="member_character">
							<img src="img/character/character1.png">
						</div>
						<p>
							<%out.print(article.getMember_id());%>
						</p>
						<p>
							<%out.print(article.getBoard_date().substring(0, 19));%>
						</p>
					</div>
					<div id="board_text">
						<%out.print(article.getBoard_text());%>
					</div>

					<form name="btns" method="post"
						action="commentWrite.bo?board_no=<%=article.getBoard_no()%>">
						<!-- 댓글 -->
						<h2>Comment</h2>
						<div id="board_comment_add">
							...
							<input type="text" name="boardComment" /> <input type="hidden"
								name="boardWriter" value="<%= article.getMember_id()%>" /> <input
						...
						<div id="boardComment_List">
							...
							<div class="boardComments" id=<%= board_comment_no%>>
								...
								<h5><%= list.get(i).getBc_date() %></h5>
								<% if(list.get(i).getMember_id().equals(member_id)){ %>
								<a href="javascript:void(0);" class="commentDelBtn">삭제</a> <a
									href="javascript:void(0);" onclick="" id="change"
									class="commentChangeBtn">수정</a>
								<% } %>
								<a href="javascript:void(0);" onclick="" class="s_comments">답글</a>
								<p><%= list.get(i).getBc_text() %></p>
								<input type="hidden" name="commentNum"
									value="<%= board_comment_no%>" /> <input type='text'
									class="changeText" name="changeText<%= i %>"
									value="<%= list.get(i).getBc_text() %>" /> <input
									type='button' name='changeBtn' value="수정" class='changeBtn'
									onClick="commentChangeBtn(<%= i %>,'<%= member_id %>','<%= list.get(i).getBc_date() %>',<%= article.getBoard_no() %>)" />
								...
								<div class="board_small_comment">
									...
									<div>
										<h5><%= member_id %></h5>
										<input type='text' class="" name="smallComment<%= i %>" /> <input
											type="button" name="" value="확인"
											onClick="location.href='smallCommentAdd.bo?boardCommentWriter=<%= list.get(i).getMember_id() %>&member_id=<%= member_id %>&board_comment_no=<%= list.get(i).getBoard_comment_no() %>&board_no=<%= list.get(i).getBoard_no() %>&bc_s_text='+ document.querySelector('input[name=smallComment<%= i %>]').value;" />
									</div>
								</div>
								<!-- 답글 리스트 -->
								<%
								...
								<div class="board_small_comment_list" id="<%= scNum%>">
									...
									<a href="javascript:void(0);" onClick="">삭제</a><a
										href=" javascript:void(0); ">수정</a>
									<%} else{	out.print("<a></a><a></a>");	}%>

									<h6><%= sc_dto.get(j).getBc_s_date().substring(0, 19) %></h6>
									<input type="text" name="sc_value"
										value="<%= sc_dto.get(j).getBc_s_text() %>"
										disabled="disabled" /> <input type="hidden" name="scNum"
										value="<%= scNum%>" /> <input type="hidden" name="bc_s_date"
										value="<%= sc_dto.get(j).getBc_s_date()%>" /> <input
										type="button" name="sc_changeBtn" value="수정" />
								</div>
								<%	}
								}
				...
				<div class="button">
					...
					<input type="button" value="목록"
						onClick="location.href='board.jsp?sitePage=<%= sitePage%>'" />
					<%	
							if ( member_id != null && member_id.equals(article.getMember_id())) { 
								request.setAttribute("article", article);							
						%>
					<input type='button' value='수정'
						onClick="location.href='boardModifyForm.bo?board_num=<%=article.getBoard_no()%>'" />
