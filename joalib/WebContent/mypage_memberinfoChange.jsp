	 ...
	 <section id="side_size">
		...
		<div id="cont_size">
			...
			<form name="memberinfo" action="memberinfoChange.mem" >
				<div id="memberNoChange"> <h5><%= dto.getMember_id() %></h5><h5> ( level. <%= dto.getMember_level() %> )</h5> </div>
				<div id="memberInfo" >
					<label for="member_name">이름</label><input type="text" name="member_name" value="<%= dto.getMember_name() %>" /><br>
					<label for="member_tel1">전화번호</label>
					<select  name="member_tel1">
						<optgroup label="Before"><option value="<%= memberTelArray[0] %>"><%= memberTelArray[0] %></option></optgroup>
						<optgroup label="Choice">
			 			...
			 			<option value="015">015</option></optgroup></select>
					-<input type="text" name="member_tel2" value="<%= memberTelArray[1] %>" />
					-<input type="text" name="member_tel3" value="<%= memberTelArray[2] %>" /><br>
					<label for="member_birth">생년월일</label><input type="date" name="member_birth" value="<%= dto.getMember_birth() %>" /><br>
					<label for="member_email_id">이메일</label><input type="text" name="member_email_id" value="<%= memberE_mailArray[0] %>" />
					<input type="text" name="member_email_adress" value="<%= memberE_mailArray[1] %>" />
					<select  name="member_email_adress_select" onchange="emailAdress(memberinfo)">
					...
					<div id = "adress">
			 			...
			 			<!-- member_city : city -->
				 		<input type="text" id="postcode" name="postcode" placeholder="우편번호" readonly="true">
						<input type="button" onclick="sample6_execDaumPostcode()" value="우편번호 찾기"><br>
						<input type="text" id="address" name="address" value="<%= memberAdressArray[0] %>" readonly="true"><br>
						<%if(memberAdressArray.length == 1){ %>
							<input type="text" id ="detailAddress" name="detailAddress" placeholder="상세주소" />		
						<%}else{	%>
							<input type="text" id ="detailAddress" name="detailAddress" value="<%= memberAdressArray[1] %>" />		
						<%} %>
					</div><br>
				</div>
				<input type="submit" value="전송" /><input type="button" value="회원탈퇴" onClick="location.href = 'mypage_memberDelete.jsp';" />
			</form>
